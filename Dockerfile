...
RUN sed -i 's/<Connector port="8080"/<Connector port="8443" protocol="org.apache.coyote.http11.Http11NioProtocol" SSLEnabled="true" scheme="https" secure="true" clientAuth="false" sslProtocol="TLS" keyAlias="'"$SSL_KEY_ALIAS"'" keystoreFile="\/usr\/local\/tomcat\/conf\/ssl.p12" keystoreType="PKCS12" keystorePass="'"$SSL_KEYSTORE_PASS"'"/' /usr/local/tomcat/conf/server.xml
